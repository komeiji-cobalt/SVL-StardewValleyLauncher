...
public partial class LaunchPageViewModel : ObservableObject
{
    ...
    {
        var customIcon = InstanceIconResolver.ResolveIconPath(instancePath);
        if (!string.IsNullOrWhiteSpace(customIcon))
        {
            return customIcon;
        }

        return ResolveDefaultInstanceIcon(isSmapiInstance);
    }
    ...
    private string ResolveDefaultInstanceIcon(bool isSmapiInstance)
    {
